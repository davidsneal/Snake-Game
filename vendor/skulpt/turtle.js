/*
 * Skulpt's turtle module (MIT, see LICENSE), patched for snake-game.
 *
 * This is src/lib/turtle.js from skulpt@1.2.0 with five additions, each marked
 * inline with a "snake-game patch" comment:
 *
 *   1. Tk/X11 colour names   -- "green4", "honeydew2" etc. are not CSS colours.
 *   2. shapesize()/turtlesize() -- missing upstream; the game is built from it.
 *   3. onkeypress()          -- missing upstream; aliased to onkey().
 *   4. key events bound to the document rather than the turtle container, so
 *      the game responds without having to click the canvas first.
 *   5. the live Screen object exposed, so the touch D-pad can reuse the key
 *      handlers the Python code registered.
 *
 * Nothing else is changed. Loaded as text by web/app.js and installed over
 * Sk.builtinFiles.files["src/lib/turtle.js"]; it is not a page script.
 *
 * To re-create against a newer Skulpt, re-apply the five marked hunks to that
 * release's src/lib/turtle.js.
 */
...
function generateTurtleModule(_target) {
    ...
    (function(proto) {
        ...
        proto.getState = function() {
            ...
            if (!this._state) {
                this._state = {
                    ...
                    /* --- snake-game patch: shapesize() support --- */
                    stretchWid   : this._stretchWid,
                    stretchLen   : this._stretchLen,
        ...
        /* --- snake-game patch: shapesize() / turtlesize() --------------------
         * Upstream Skulpt has no shapesize(), so every turtle draws at the
         * default 20x20. snake.py builds its entire board out of scaled squares
         * -- the 630px playing field, the border, the snake segments, the eyes
         * and pupils -- so without this the game is just a pile of identical
         * squares. Semantics follow CPython's turtle.TPen.shapesize(): a lone
         * argument sets both factors, and a bare call reports the current ones.
         * -------------------------------------------------------------------- */
        proto.$shapesize = proto.$turtlesize = function (stretch_wid, stretch_len, outline) {
            if (stretch_wid === undefined && stretch_len === undefined && outline === undefined) {
                return [this._stretchWid, this._stretchLen, this._outlineWidth];
            }

            if (stretch_wid === 0 || stretch_len === 0) {
                throw new Sk.builtin.ValueError("stretch_wid/stretch_len must not be zero");
            }

            if (stretch_wid !== undefined) {
                this._stretchWid = stretch_wid;
                this._stretchLen = (stretch_len === undefined) ? stretch_wid : stretch_len;
            }
            else if (stretch_len !== undefined) {
                this._stretchLen = stretch_len;
            }
    ...
    (function(proto) {
        ...
        proto.reset = function() {
            ...
            /* --- snake-game patch: unbind from the document, see below --- */
        ...
        proto._createKeyDownListener = function() {
            ...
            /* --- snake-game patch: listen on the document ------------------
             * Upstream binds to the turtle container, which only receives key
             * events while it happens to hold focus. On a normal web page that
             * means the arrow keys do nothing until you click the canvas.
             * Binding to the document makes the game respond immediately.
             * preventDefault() is still only called for keys the script has
             * actually bound, so the rest of the page keeps working.
             * --------------------------------------------------------------- */
        ...
        /* --- snake-game patch: onkeypress() ----------------------------------
         * Upstream only exposes onkey(). CPython's turtle splits key handling
         * into onkeypress() (key-down, with auto-repeat) and onkeyrelease()
         * (key-up); Skulpt's onkey() is already wired to key-down through
         * _createKeyDownListener and _createKeyRepeater, which is exactly
         * onkeypress semantics, so the two can share one implementation.
         * -------------------------------------------------------------------- */
    ...
    function getScreen() {
        if (!_screenInstance) {
            ...
            /* --- snake-game patch: expose the live Screen so the on-screen
             * D-pad can fire the same handlers the arrow keys do. --- */
            if (Sk.TurtleGraphics) {
                Sk.TurtleGraphics.screenInstance = _screenInstance;
            }
        }
    ...
    function drawTurtle(state, context) {
        var shape  = SHAPES[state.shape],
            ...
            xScale = world.xScale,
            yScale = world.yScale,
            x, y, bearing, stretchWid, stretchLen;
        ...
        if (shape.nodeName) {
            ...
            /* --- snake-game patch: apply shapesize() stretch factors ---------
             * After the rotate above, local x runs across the turtle's heading
             * and local y runs along it, which is how CPython defines
             * stretch_wid and stretch_len. The outline is a pixel width, so it
             * has to be divided back out of the scale we just applied.
             * ---------------------------------------------------------------- */
            stretchWid = (state.stretchWid === undefined) ? 1 : state.stretchWid;
            stretchLen = (state.stretchLen === undefined) ? 1 : state.stretchLen;
            context.scale(stretchWid, stretchLen);
            context.beginPath();
            context.lineWidth   = (state.outlineWidth === undefined ? 1 : state.outlineWidth) /
                                  (Math.sqrt(Math.abs(stretchWid * stretchLen)) || 1);
    ...
    /* --- snake-game patch: Tk/X11 colour names -------------------------------
     * Python's turtle is a Tk wrapper, so turtle scripts use Tk colour names.
     * Many of them -- "green4", "honeydew2", "DodgerBlue4", every "<name><1-4>"
     * brightness variant -- are not CSS colours. Assigning one to a canvas
     * fillStyle is silently ignored, so the shape keeps whatever colour was set
     * before it, which is how an unpatched Skulpt renders this game in black.
     * Resolve them through the X11 table in web/tk-colors.js instead.
     *
     * Where X11 and CSS disagree on a shared name (green, purple, maroon, gray)
     * the X11 value wins, because that is what the script saw under Tk.
     * ------------------------------------------------------------------------ */
    function resolveTkColor(color) {
        var table = (typeof window !== "undefined" && window.TK_COLORS) || {},
        ...
        // gray0..gray100 / grey0..grey100 are percentages, not table entries.
        if ((grey = /^gr([ae])y(\d{1,3})$/.exec(key))) {
            level = Math.round(Math.min(100, parseInt(grey[2], 10)) * 255 / 100);
    ...
    function createColor(turtleColorMode,color, g, b, a) {
        ...
        if (color.constructor === Array && color.length) {
            ...
            /* --- snake-game patch: resolve Tk colour names --- */
            color = resolveTkColor(color.replace(/\s+/g, ""));
        }
        else {
            return "black";
        }
